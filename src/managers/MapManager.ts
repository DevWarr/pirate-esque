...
export class MapManager {
  ...
  public static get tileMapSize(): SizeVector2 {
    return {
      w: MapManager.terrainMap[0].length,
      h: MapManager.terrainMap.length,
    };
  }

  private static getTexture(tileMap: MapTileKey[][], x: number, y: number) {
    const terrainTileType: TerrainTileType = tileMap[y][x][MapManager.mapKeyIndex] as TerrainTileType;
  ...
  public static buildSpriteMapFromMapTiles(tileMap: MapTileKey[][]): void {
    this.terrainMap = tileMap.map((tileMapRow, yPosition) =>
      tileMapRow.map((_, xPosition) => {
        return {
          tileType: tileMap[yPosition][xPosition][MapManager.mapKeyIndex] as TerrainTileType,
          sprite: new Sprite({
  ...
  public static isLand(position: PositionVector2): boolean {
    return MapManager.terrainMap[position.y][position.x].tileType === TerrainTileType.LAND;
  }
